...
var div2 = dom.create("<div>childNode</div>");
var div3 = dom.create("<div id ='parent'></div>");
var div4 = dom.create("<div id ='find'></div>");
var div5 = dom.create("<div id ='find'></div>");
console.log(div); //增
...
dom.remove(div2);
var e = dom.empty(div3);
console.log(e); //输出删除的子元素
//改

dom.attr(div3, "title", "kerry"); //改属性

var title = dom.attr(div3, "title");
console.log("title:".concat(title));
dom.text(div, "我是新的text"); //改文本内容

var text = dom.text(div); //显示文本内容

console.log(text); //改style样式的两种方式

dom.style(div, "color", "red");
dom.style(div, {
  color: "green"
});
var color = dom.style(div, "color");
console.log(color); //改class属性

dom.class.add(div3, "red");
dom.class.remove(div3, "red");
var class1 = dom.class.has(div3, "red");
console.log("class1:".concat(class1));
...
var parentDiv = dom.find("#parent")[0]; //这个[0]千万别忘

console.log(parentDiv); //查元素

dom.append(div, div4);
dom.append(div3, div5);
var f = dom.find("#find", div)[0];
console.log(f); //查找兄弟元素

var s2 = dom.find("#s2")[0];
console.log(dom.siblings(s2)); //查找下一个节点

console.log(dom.next(s2)); //找上一个节点

console.log(dom.previous(s2)); //遍历节点
